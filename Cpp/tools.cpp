...
std::string formattedDuration(const double &duration) {
	std::ostringstream oss;
	if (duration < 60.) oss << FIXED(0) << duration << " s";
	else if (duration < 3600.) oss << FIXED(2) << duration/60. << " min";
	else if (duration < 86400.) oss << FIXED(2) << duration/3600. << " h";
	else oss << FIXED(3) << duration/86400. << " d";
	return oss.str();
}
