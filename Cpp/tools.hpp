// (c) 2018-present Pttn (https://riecoin.xyz/StellaPool-Mini)
...
#define LOGMSG(message) std::cout << "[" << timeNowStr() << "] " << message << std::endl
#define ERRORMSG(message) std::cerr << "[" << timeNowStr() << "] E: "  << __func__ << ": " << message << std::endl
...
std::string timeStr(const uint64_t, const bool = false);
inline std::string timeNowStr(const bool simple = false) {return timeStr(nowU64(), simple);}
